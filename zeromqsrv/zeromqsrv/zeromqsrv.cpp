...
struct worker {
    ...
    worker(std::string identity, std::shared_ptr<service>  service = 0, int64_t expiry = 0) {
        m_identity = identity;
        m_service = service;
        m_expiry = expiry;
    }
...
private:
    ...
    std::shared_ptr<service> 
        ...
        if (m_services.count(name)) {
            ...
            std::shared_ptr<service>  srv = new service(name);
    ...
    void
        service_dispatch(std::shared_ptr<service> srv, zmsg *msg) {
        assert(srv);
    ...
    void
        ...
        } else if (service_name.compare("mmi.echo") == 0) {
        } else {
            msg->body_set("501");
        }
